...
class ProfileController extends BaseController
{
	...
	public function updateLanguagesAction(Request $request, $userName)
	{
		$user = $this->container->get('security.context')->getToken()->getUser();
		if (!is_object($user) || !$user instanceof UserInterface)
		{
	...
	public function updateLanguageAction(Request $request, $userName, $languageid)
	{
		$user = $this->container->get('security.context')->getToken()->getUser();
		if (!is_object($user) || !$user instanceof UserInterface)
		{
	...
	public function deleteLanguageAction(Request $request, $userName, $languageid)
	{
		$user = $this->container->get('security.context')->getToken()->getUser();
		if (!is_object($user) || !$user instanceof UserInterface)
		{
	...
	public function addLanguageAction(Request $request, $userName)
	{
		$user = $this->container->get('security.context')->getToken()->getUser();
		if (!is_object($user) || !$user instanceof UserInterface)
		{
		...
		/** @var \IMDC\TerpTubeBundle\Entity\Language $language   */
		$language = new Language();
		
		$formFactory = $this->container->get('form.factory');
		
		$form = $formFactory->create(new LanguageFormType(), $language, array());
		if ('POST' === $request->getMethod())
		{
			...
			if ($form->isValid())
			{
				...
				$this->container->get('session')->getFlashBag()->add('language', 'Language added successfully!');
		...
		return $this->container->get('templating')
		->renderResponse(
				'IMDCTerpTubeBundle:Profile:edit_language.html.'
				. $this->container->getParameter('fos_user.template.engine'),
