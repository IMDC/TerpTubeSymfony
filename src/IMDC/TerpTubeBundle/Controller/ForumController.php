...
class ForumController extends Controller
{
	...
	public function listAction(Request $request)
	{
		...
		$forums = $paginator->paginate ( $forums, $request->query->get ( 'page', 1 ), /*page number*/
			8 /*limit per page*/
		);
	...
	public function newAction(Request $request, $groupId)
	{
		...
		else
		{
			...
			if ($media)
			{
				...
				/*
				 * if (!$forum->getTitleMedia()->contains($media)) $forum->addTitleMedia($media);
				 */
			...
			$this->get ( 'session' )->getFlashBag ()->add ( 'info', 'Forum created successfully!' );
	...
	public function viewAction(Request $request, $forumid)
	{
		...
		$paginator = $this->get ( 'knp_paginator' );
		$threads = $paginator->paginate ( $threads, $request->query->get ( 'page', 1 ) /* page number */,
	    	8 /* limit per page */
	    );
	...
	public function editAction(Request $request, $forumid)
	{
		...
		else
		{
			...
			$this->get ( 'session' )->getFlashBag ()->add ( 'info', 'Forum edited successfully!' );
	...
	public function deleteAction(Request $request, $forumid)
	{
		...
		if ($form->isValid ())
		{
			$user = $this->getUser ();
			/*
			 * $threads = $forum->getThreads(); foreach ($threads as $thread) { $threadposts = $thread->getPosts(); foreach ($threadposts as $threadpost) { $threadpost->getAuthor()->removePost($threadpost); $em->remove($threadpost); } $thread->getCreator()->removeThread($thread); $em->remove($thread); }
			 */
			...
			$this->get ( 'session' )->getFlashBag ()->add ( 'info', 'Forum deleted successfully!' );
