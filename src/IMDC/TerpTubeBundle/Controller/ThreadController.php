...
class ThreadController extends Controller
{
	public function indexAction()
	{
		...
		$response = $this->render('IMDCTerpTubeBundle:Thread:index.html.twig', array('threads' => $threads));
		return $response;
	...
	public function viewThreadAction(Request $request, $threadid)
	{
		...
		$newpost = new Post();
		$postform = $this
				->createForm(new PostFormFromThreadType(), $newpost, array('user' => $user, 'em' => $em,));
		$em = $this->getDoctrine()->getManager();
		...
		if ($postform->isValid())
		{
			...
			$this->get('session')->getFlashBag()->add('notice', 'Post created successfully!');
			...
			// create a new form
			$post = new Post();
			$form = $this->createForm(new PostFormFromThreadType(), $post, array('user' => $user, 'em' => $em,));

			return $this
					->render('IMDCTerpTubeBundle:Thread:viewthread.html.twig',
							array('form' => $form->createView(), 'thread' => $thread, 'threadposts' => $threadposts,));
		}

		// form not valid, show the thread
		return $this
				->render('IMDCTerpTubeBundle:Thread:viewthread.html.twig',
						array('form' => $postform->createView(), 'thread' => $thread, 'threadposts' => $threadposts,));
	}
	...
	public function createNewThreadAction(Request $request)
	{
		...
		$form = $this->createForm(new ThreadFormType(), $newthread, array('user' => $this->getUser(),));
		$em = $this->getDoctrine()->getManager();
		/*
		...
		if ($form->isValid())
		{
			...
			$this->get('session')->getFlashBag()->add('notice', 'Thread created successfully!');
			return $this->redirect($this->generateUrl('imdc_thread_show_recent'));
		}

		// form not valid, show the basic form
		return $this->render('IMDCTerpTubeBundle:Thread:new.html.twig', array('form' => $form->createView(),));
	}
	...
	public function createNewThreadFromMediaAction(Request $request, $resourceid)
	{
		...
		$form = $this
				->createForm(new ThreadFromMediaFormType(), $newthread,
						array('user' => $this->getUser(), 'resource' => $chosenmedia,));
		...
		return $this
				->render('IMDCTerpTubeBundle:Thread:newfrommedia.html.twig',
						array('form' => $form->createView(), 'mediaFile' => $chosenmedia,));
	}
