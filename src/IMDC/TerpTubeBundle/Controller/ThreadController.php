...
class ThreadController extends Controller
{
	...
	public function newAction(Request $request, $forumid, $mediaId)
	{
		...
		else
		{
			...
			if ($media)
			{
				...
				/*
				 * if (!$thread->getMediaIncluded()->contains($media)) { $thread->addMediaIncluded($media); $thread->setType($media->getType()); }
				 */
			...
			$this->get ( 'session' )->getFlashBag ()->add ( 'success', 'Thread created successfully!' );
	...
	public function editAction(Request $request, $threadid)
	{
		...
		else
		{
			...
			/*
			 * $media = $form->get('mediatextarea')->getData(); if ($media) { if (!$user->getResourceFiles()->contains($media)) { throw new AccessDeniedException(); //TODO more appropriate exception? } if (!$thread->getMediaIncluded()->contains($media)) { $thread->addMediaIncluded($media); $thread->setType($media->getType()); } }
			 */
			...
			$this->get ( 'session' )->getFlashBag ()->add ( 'success', 'Forum post edited successfully!' );
	...
	public function deleteAction(Request $request, $threadid)
	{
		...
		if ($form->isValid ())
		{
			...
			$this->get ( 'session' )->getFlashBag ()->add ( 'success', 'Post successfully deleted!' );
