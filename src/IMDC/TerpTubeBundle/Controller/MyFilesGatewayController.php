...
class MyFilesGatewayController extends Controller
{
    ...
    public function listAction (Request $request)
    {
        ...
        $response = $this->render(
                'IMDCTerpTubeBundle:MyFiles:' . ($request->isXmlHttpRequest() ? 'ajax.' : '') . 'index.html.twig', 
                $parameters);
    ...
    public function gatewayInterpretationsAction (Request $request)
    {
        ...
        return $this->render('IMDCTerpTubeBundle:MyFilesGateway:interpretations.html.twig', 
                array(
    ...
    public function previewCompoundMediaAction (Request $request, $compoundMediaId, $url)
    {
        ...
        if (! $request->isXmlHttpRequest())
            return $this->render('IMDCTerpTubeBundle:MyFilesGateway:previewCompoundMedia.html.twig', 
                    array(
                            "compoundMedia" => $mediaFile
                    ));
        else
            $response = $this->render('IMDCTerpTubeBundle:MyFilesGateway:ajax.previewCompoundMedia.html.twig', 
                    array(
                            "compoundMedia" => $mediaFile
                    ));
        ...
        if ($media == null)
        {
            ...
            else
            {
                ...
                if ($media->getIsReady() == Media::READY_YES)
                {
                    ...
                    $videoDuration = $ffprobe->streams($finalFile->getRealPath())
                        ->videos()
                        ->first()
                        ->get('duration');
    ...
    public function addRecordingAction (Request $request, $url)
    {
        if (! $request->isMethod('POST'))
        {
            ...
            $response = $this->render('IMDCTerpTubeBundle:Media:' . $prefix . 'recordVideo.html.twig', 
                    array(
                            "recorderConfiguration" => $recorderConfiguration,
                            'isPost' => $this->get('request')
                                ->get('isPost', false)
                    ));
        ...
        $finalFile = $isFirefox == 'false' ? $transcoder->mergeAudioVideo($audioFile, $videoFile) : $transcoder->remuxWebM(
                $audioFile);
    ...
    public function addSimultaneousRecordingAction (Request $request, $sourceMediaID, $startTime, $url)
    {
        if (! $request->isMethod('POST'))
        {
            ...
            return $this->render('IMDCTerpTubeBundle:MediaController:simultaneousPreviewAndRecord.html.twig', 
                    array(
                            "mediaFile" => $mediaFile
                    ));
        }
    ...
    { // TODO delete
        ...
        $response = $this->render('IMDCTerpTubeBundle:AddFileGateway:' . $prefix . 'addFile.html.twig', 
                array(
