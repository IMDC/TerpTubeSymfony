...
class MessageController extends Controller
{
	...
	public function newAction(Request $request, $username = null, $recipients = null)
	{
		...
		if (! $form->isValid ())
		{
			if ($username)
			{
				$user = $em->getRepository ( 'IMDCTerpTubeBundle:UserGroup' )->findOneBy ( array (
						'username' => $username 
				) );
				if ($user)
				{
					$form->get ( 'recipients' )->setData ( array (
							$user 
					) );
				}
			}
	 ...
	 * Doesn't actually delete the message, as messages
	...
	public function deleteMessageAction(Request $request, $messageid)
	{
		...
		{
			throw $this->createNotFoundException ( 'User is not found in the recipients for this message id: ' . $messageid );
		}
	...
	public function archiveMessageAction(Request $request, $messageid)
	{
		...
		$this->get ( 'session' )->getFlashBag ()->add ( 'inbox', 'Message id:' . $message->getId () . ' successfully archived' );
		return $this->redirect ( $this->generateUrl ( 'imdc_message_inbox' ) );
	}
	...
	public function viewMessageAction(Request $request, $messageid)
	{
		...
		if ($message->getRecipients ()->contains ( $user ) || $message->getOwner () == $user)
		{
			
			// is this message deleted already?
			if ($user->getDeletedMessages ()->contains ( $message ))
			{
				$this->get ( 'session' )->getFlashBag ()->add ( 'danger', 'This message has been deleted' );
				return $this->redirect ( $this->generateUrl ( 'imdc_message_inbox' ) );
			}
		...
		{
			$this->get ( 'session' )->getFlashBag ()->add ( 'danger', 'You do not have permission to view this message' );
			return $this->redirect ( $this->generateUrl ( 'imdc_message_inbox' ) );
		}
	}
