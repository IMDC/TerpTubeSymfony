...
class Transcoder
{
	...
	{
		if ($this->ffprobe->streams ( $inputFile->getRealPath () )->audios ()->count () != 0)
			return true;
		else
			return false;
	}
	...
	{
		if ($this->ffprobe->streams ( $inputFile->getRealPath () )->videos ()->count () != 0)
			return true;
		else
			return false;
	}
	...
	public function transcodeToWebM(File $inputFile, $preset)
	{
		...
		try
		{
			...
			{
				$hasAudio = true;
				if ($this->ffprobe->streams ( $inputFile->getRealPath () )->audios ()->first ()->has ( 'bit_rate' ))
					$audioBitRate = min ( $audioBitRate, intval ( $this->ffprobe->streams ( $inputFile->getRealPath () )->audios ()->first ()->get ( 'bit_rate' ) ) );
