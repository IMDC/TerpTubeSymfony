define(
	...
	function(MediaManager)
	{
		...
		this.bind__onLoadPageSuccess = this._onLoadPageSuccess
			.bind(this);
		this.bind__onRecordingSuccess = this._onRecordingSuccess
			.bind(this);
		...
		this.bind__doneAndPostFunction = this._doneAndPostFunction
			.bind(this);
		this.bind__forwardFunctionCut = this._forwardFunctionCut
			.bind(this);
		this.bind__backFunction = this._backFunction.bind(this);
		this.bind__loadSelectFromMyFilesFunction = this._loadSelectFromMyFilesFunction
			.bind(this);
		this.bind__terminatingFunction = this._terminatingFunction
			.bind(this);

		this.bind__onClickRemoveSelectedMedia = this._onClickRemoveSelectedMedia
			.bind(this);
	    };
	    ...
	    // this must be the same name defined in
	    // {bundle}/Form/Type/MediaType
		...
		if (typeof this.uploadForm === "undefined")
		    this.uploadForm = $(this._getElement(
			    MediaChooser.Binder.CONTAINER_UPLOAD).data("form"));
		...
		return this.getForm().find(
			"#" + MediaChooser.FORM_NAME + "_" + fieldName);
	    };
		...
		/*
		 * if (this.bindBlocked) { console.log("%s: %s-
		 * bindBlocked=true", MediaChooser.TAG, "bindUIEvents");
		 * this.bindRequested = true; return; }
		 */

		this.modalDialog = this._getElement(
			MediaChooser.Binder.MODAL_DIALOG).modal(
		{
		    show : false
		});
		this.modalDialog.on("hidden.bs.modal",
			this.bind__terminatingFunction);

		this
			._getElement(MediaChooser.Binder.RECORD_VIDEO)
			.on(
				"click",
				(function(e)
				{
				    ...
				    this
						...
						showPopup : true,
						url : Routing
							.generate("imdc_myfiles_add_recording"),
						method : "GET"
					    });
				}).bind(this));

		this._getElement(MediaChooser.Binder.UPLOAD_FILE).on("click",
			(function(e)
			{
			    e.preventDefault();
		...
		this._getElement(MediaChooser.Binder.SELECT).on("click",
			(function(e)
			{
			    e.preventDefault();
		...
		console.log("%s: %s", MediaChooser.TAG,
			"_bindUIEventsSelectFromMyFiles");
		...
		container.find(".delete-button").on(
			"click",
			(function(e)
			{
			    e.preventDefault();
			    ...
			    $(this.mediaManager).one(
				    MediaManager.EVENT_DELETE_ERROR,
				    function(error, e)
				    {
					if (e.status == 500)
					{
			    ...
			    return this.mediaManager.deleteMedia(file
				    .data("val"), $(
				    "#mediaDeleteConfirmMessage").html());
			}).bind(this));

		container.find(".select-button").on(
			"click",
			(function(e)
			{
			    e.preventDefault();

			    var isSelected = $.grep(this.media, function(
				    elementOfArray, indexInArray)
			    {
				return elementOfArray.id == $(e.target).data(
					"val");
			    }).length > 0;
		...
		this.modalDialog.find(".modal-title").html(
			this._getPopupDialogTitle());
				    ...
				    .addEventListener(
						...
						MediaChooser
							._updateUploadProgress(
								this
									._getElement(MediaChooser.Binder.CONTAINER_UPLOAD),
								Math
	    ...
	    MediaChooser.prototype._onLoadPageSuccess = function(data,
		    textStatus, jqXHR)
	    {
		console.log("%s: %s- finished=%s", MediaChooser.TAG,
			"_onLoadPageSuccess", data.finished);
		...
		{
		    payload : data
		// FIXME some event listeners my still use 'data' instead of
		// 'payload'. 'data' is reserved by jquery
		}));
	    };
		...
		this.recorder = new Player(
			$(MediaChooser.Binder.CONTAINER_RECORD_VIDEO),
			{
		...
		$(this.recorder)
			.on(
				...
				function(e, percentComplete)
				{
				    MediaChooser
					    ._updateUploadProgress(
						    $(MediaChooser.Binder.CONTAINER_UPLOAD_VIDEO_RECORDING),
						    percentComplete);
				});

		$(this.recorder)
			.on(
				Player.EVENT_RECORDING_UPLOADED,
				function(data)
				{
				    $(
					    MediaChooser.Binder.CONTAINER_UPLOAD_VIDEO_RECORDING)
	    ...
	    {
		console.log("%s: %s- mediaId=%d", MediaChooser.TAG,
			"_onRecordingSuccess", data.media.id);
	    ...
	    {
		console.log("%s: %s- e=%s", MediaChooser.TAG,
			"_onRecordingError", e);
	    };
		...
		this.player.setCurrentMinMaxTime(currentMinMaxTimes.minTime,
			currentMinMaxTimes.maxTime);

		console.log("Current Min/Max Times %s %s",
			currentMinMaxTimes.minTime, currentMinMaxTimes.maxTime);
		console.log("Cutting to Min/Max Times %s %s",
			currentMinMaxTimes.minTime
				- previousMinMaxTimes.minTime,
			currentMinMaxTimes.maxTime
				- previousMinMaxTimes.minTime);

		this.mediaManager.trimMedia(this.activeMedia.id,
			currentMinMaxTimes.minTime
				- previousMinMaxTimes.minTime,
			currentMinMaxTimes.maxTime
				- previousMinMaxTimes.minTime);
		...
		if (this._getElement(MediaChooser.Binder.SELECTED_MEDIA)
			.filter("[data-mid='" + this.activeMedia.id + "']").length == 0)
		{
		...
		newSelectedMedia = newSelectedMedia
			.replace(/__id__/g, media.id);
		newSelectedMedia = newSelectedMedia.replace(/__title__/g,
			media.title);
		newSelectedMedia = newSelectedMedia.replace(
			/__resource_webPath__/g, media.resource.pathMPEG);
		container.append(newSelectedMedia);

		this._getElement(MediaChooser.Binder.SELECTED_MEDIA).filter(
			"[data-mid='" + media.id + "']").find(
			MediaChooser.Binder.REMOVE).on("click",
			this.bind__onClickRemoveSelectedMedia);
	    };

	    MediaChooser.prototype._removeSelectedMedia = function(mediaId)
	    {
		this._getElement(MediaChooser.Binder.SELECTED_MEDIA).filter(
			"[data-mid='" + mediaId + "']").remove();
	    };
	    ...
	    {
		this._getElement(MediaChooser.Binder.RECORD_VIDEO).attr(
			'disabled', disabled);
		this._getElement(MediaChooser.Binder.UPLOAD_FILE).attr(
			'disabled', disabled);
		this._getElement(MediaChooser.Binder.SELECT).attr('disabled',
			disabled);
	    };
		...
		this.container = typeof container != "undefined" ? container
			: this.container;
	    };
		...
		if (mediaIds.length > 0)
		{
			...
			{
			    // console.log("%s: %s: %s", Post.TAG, "handlePage",
			    // "success");
			    ...
			    this._getElement(MediaChooser.Binder.WORKING)
				    .hide();
			    this._toggleForm(false);

			    /*
			     * this.bindBlocked = false; if (this.bindRequested) {
			     * this.bindUIEvents(); }
			     */

			    $(this).trigger(
				    $.Event(MediaChooser.Event.SUCCESS, {}));
			}).bind(this),
